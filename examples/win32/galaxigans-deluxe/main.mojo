...
def hall_path() raises -> String:
    # LOCALAPPDATA is where a Windows game keeps this; a profile without
    # one falls back beside the executable.
    var home = get_environment("LOCALAPPDATA")
    if home.byte_length() == 0:
        return String(".galixigans-deluxe-hall")
...
struct Game(Movable):
    ...
    def __init__(out self, mut sprites: Sprites) raises:
        ...
        let mine_def = define_mine_sprite(sprites)
        let boss_def = define_boss_sprite(sprites)
        let pilot_def = define_pilot_sprite(sprites)
        let ship_def = define_ship_sprite(sprites)
        let saucer_def = define_saucer_sprite(sprites)
...
def main() raises:
    ...
    var sky_body_bytes = _bytes_of(COSMOS_SHADER)
    var cosmos = ShaderPane(pane.ctx, pane.device, pane.context, sky_body_bytes)      # layer 0: the twelve cosmos
    cosmos.set_aspect(pane.aspect())
    var field = IndexedPane(pane.ctx, pane.device, pane.context, FIELD_W, FIELD_H, FIELD_W, FIELD_H)
    ...
    var hud = TextOverlay(pane.ctx, pane.device, pane.context, FIELD_W, FIELD_H)
    var game = Game(sprites)
    ...
    while pane.pump():
        ...
        cosmos.render(f, pane.rtv)  # layer 0: Clear -- the cosmos
        field.render(f, pane.rtv)   # layer 1: bullets, bombs, sparks, the beam
        sprites.render(f, pane.rtv, 0.0, 0.0, Float64(FIELD_W), Float64(FIELD_H))
        hud.render(f, pane.rtv)     # layer 3: the text
        pane.end_frame(f)
    stop_tune_gm()
    stop_audio(unit)
    deck_free(deck)
